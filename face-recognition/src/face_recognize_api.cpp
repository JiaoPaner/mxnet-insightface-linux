...
void test(char* base_src, char* target_src) {
    ...
    Mat base_emb = recognizer.extractFeature(recognizer.createAlignFace(base,1)[0]);
    ////std::cout << base_emb << std::endl;
    Mat target_emb = recognizer.extractFeature(recognizer.createAlignFace(target,1)[0]);
